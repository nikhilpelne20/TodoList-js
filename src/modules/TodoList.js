...
export default class TodoList {
    ...
    addProject(project) {
        if (
            this.projects.some(
                (existingProject) => existingProject.getName() === project.name,
            )
        ) {
            return;
        }
    ...
    contains(project) {
        return this.projects.some(
            (existingProject) => existingProject.getName() === project,
        );
    }

    deleteProject(projectName) {
        const project = this.projects.find(
            (allProject) => allProject.getName() === projectName,
        );
    ...
    getUpcomingProject() {
        ...
        this.projects.forEach((project) => {
            ...
            upcomingTask.forEach((task) => {
                const taskName = `${task.getName()}(${project.getName()})`;
                this.getProject('Upcoming').addTask(
                    new Task(taskName, task.getDate()),
                );
            });
        });

        this.getProject('Upcoming').setTasks(
